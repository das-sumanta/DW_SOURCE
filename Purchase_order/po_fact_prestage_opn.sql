...
SELECT A.document_number,
       ...
       X.DATE_KEY AS TRANSACTION_DATE_KEY,
...
FROM dw_prestage.po_fact_insert A
  ...
  INNER JOIN DW_REPORT.SUBSIDIARIES G ON (A.SUBSIDIARY_ID = G.SUBSIDIARY_ID)
  ...
  INNER JOIN DW_REPORT.DWDATE X ON (TO_CHAR (A.TRANDATE,'YYYYMMDD') = X.DATE_ID)
...
A.VENDOR_ID,
  CASE
         ...
         WHEN ( B.VENDOR_KEY IS NULL AND A.VENDOR_ID IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END, 
A.REQUESTOR_ID,
  CASE
...
CASE
       ...
       END,
CASE
         WHEN ( F.DATE_KEY IS NULL AND A.CREATE_DATE IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN ( F.DATE_KEY IS NULL AND A.CREATE_DATE IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END,
A.SUBSIDIARY_ID,
CASE
         WHEN (G.subsidiary_key IS NULL AND A.SUBSIDIARY_ID IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN (G.subsidiary_key IS NULL AND A.SUBSIDIARY_ID IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END,
A.LOCATION_ID,
CASE
...
CASE
         ...
         WHEN (K.CURRENCY_KEY IS NULL AND A.CURRENCY_ID IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END,
A.CLASS_ID,
CASE
...
CASE
         ...
         WHEN (W.transaction_type_key IS NULL AND A.custom_form_id IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END,
...
A.TRANDATE,
CASE
         WHEN ( X.DATE_KEY IS NULL AND A.TRANDATE IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN ( X.DATE_KEY IS NULL AND A.TRANDATE IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END AS TRANDATE_ERROR,
X.DATE_KEY AS TRANSACTION_DATE_KEY,
...
FROM dw_prestage.po_fact_insert A
  ...
  LEFT OUTER JOIN DW_REPORT.DWDATE X ON (TO_CHAR (A.TRANDATE,'YYYYMMDD') = X.DATE_ID)
WHERE A.LINE_TYPE = 'PO_LINE'
AND   (B.VENDOR_KEY IS NULL OR 
       ...
       F.DATE_KEY IS NULL OR X.DATE_KEY IS NULL OR G.SUBSIDIARY_KEY IS NULL OR 
       ( L.LOCATION_KEY IS NULL AND A.LOCATION_ID IS NOT NULL ) OR 
       ( R.DEPARTMENT_KEY IS NULL AND A.DEPARTMENT_ID IS NOT NULL ) OR 
       ( M.ITEM_KEY  IS NULL AND A.ITEM_ID IS NOT NULL ) OR 
       ( Q.TAX_ITEM_KEY  IS NULL AND A.TAX_ITEM_ID IS NOT NULL ) OR  
