...
FROM dw_prestage.vra_fact_insert A
  ...
  INNER JOIN DW_REPORT.EMPLOYEES C ON (NVL (A.CREATED_BY_ID,-99) = C.EMPLOYEE_ID)
  INNER JOIN DW_REPORT.DWDATE F ON (NVL (TO_CHAR (A.CREATE_DATE,'YYYYMMDD'),'0') = F.DATE_ID)
  INNER JOIN DW_REPORT.DWDATE H ON (NVL (TO_CHAR (A.SHIPMENT_RECEIVED_DATE,'YYYYMMDD'),'0') = H.DATE_ID)
  INNER JOIN DW_REPORT.DWDATE N ON (NVL (TO_CHAR (A.CLOSE_DATE,'YYYYMMDD'),'0') = N.DATE_ID)
...
FROM dw_prestage.vra_fact_insert A
  ...
  LEFT OUTER JOIN DW_REPORT.EMPLOYEES C ON (NVL (A.CREATED_BY_ID,-99) = C.EMPLOYEE_ID)
  LEFT OUTER JOIN DW_REPORT.DWDATE F ON (NVL (TO_CHAR (A.CREATE_DATE,'YYYYMMDD'),'0') = F.DATE_ID)
  LEFT OUTER JOIN DW_REPORT.DWDATE H ON (NVL (TO_CHAR (A.SHIPMENT_RECEIVED_DATE,'YYYYMMDD'),'0') = H.DATE_ID)
  LEFT OUTER JOIN DW_REPORT.DWDATE N ON (NVL (TO_CHAR (A.CLOSE_DATE,'YYYYMMDD'),'0') = N.DATE_ID)
