SELECT 1 as runid,
       TO_CHAR(TAX_ITEMS.ACCOUNT_CLASSIFICATION_ROYA_ID) AS ACCOUNT_CLASSIFICATION_ROYA_ID,
       APPLY_FRIEGHT,
       ...
       TO_CHAR(TAX_ITEMS.ISBN_ANZ_ID) AS ISBN_ANZ_ID,
       ISINACTIVE,
       ...
       REPLACE(REPLACE(TAX_ITEMS.JDE_ITEM_CODE,CHR(10),' '),CHR(13),' ') AS JDE_ITEM_CODE,
       TO_CHAR(TAX_ITEMS.LAUNCH_DATE,'YYYY-MM-DD HH24:MI:SS') AS LAUNCH_DATE,
       LENGTH_CM,
       TO_CHAR(TAX_ITEMS.LEVEL_ID) AS LEVEL_ID,
       LEXILE,
       REPLACE(REPLACE(TAX_ITEMS.MINIMUM_PURCHASE_QUANTITY,CHR(10),' '),CHR(13),' ') AS MINIMUM_PURCHASE_QUANTITY,
       ...
       TO_CHAR(TAX_ITEMS.PLANNER_NEW_ID) AS PLANNER_NEW_ID,
       TO_CHAR(TAX_ITEMS.PLANNER_NUMBER_ID) AS PLANNER_NUMBER_ID,
       TO_CHAR(TAX_ITEMS.PRODUCT_CALSSIFICATION_ANZ_ID) AS PRODUCT_CALSSIFICATION_ANZ_ID,
       ...
       SOR_TAG,
       TO_CHAR(TAX_ITEMS.SUBSTITUTE_ITEM_ID) AS SUBSTITUTE_ITEM_ID,
	 ...
	 TAX_ITEMS,
	 ACCOUNTS
	 WHERE TAX_ITEMS.DATE_LAST_MODIFIED > '1900-01-01 00:00:00'
	 AND TAX_ITEMS.INCOME_ACCOUNT_ID = ACCOUNTS.ACCOUNT_ID(+);
