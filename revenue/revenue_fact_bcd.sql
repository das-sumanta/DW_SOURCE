...
SELECT b.tranid AS document_number,
       ...
       TO_CHAR(NVL(b.sales_rep_id,h.sales_rep_id)) AS sales_rep_id,
       TO_CHAR(B.SALES_TERRITORY_ID) AS SALES_TERRITORY_ID,
       NULL AS BILL_ADDRESS_LINE_1,
       NULL AS BILL_ADDRESS_LINE_2,
       NULL AS BILL_ADDRESS_LINE_3,
       NULL AS BILL_CITY,
       NULL AS BILL_COUNTRY,
       NULL AS BILL_STATE,
       NULL AS BILL_ZIP,
       NULL AS SHIP_ADDRESS_LINE_1,
       NULL AS SHIP_ADDRESS_LINE_2,
       NULL AS SHIP_ADDRESS_LINE_3,
       NULL AS SHIP_CITY,
       ...
       TO_CHAR(B.entity_ID) AS customer_ID,
       TO_CHAR(a.price_type_ID) AS price_type_ID,
       TO_CHAR(NVL(B.custom_form_ID,1)) AS custom_form_ID,
       TO_CHAR(b.created_by_ID) AS created_by_ID,
       TO_CHAR(B.create_date,'YYYY-MM-DD HH24:MI:SS') AS create_date,
       TO_CHAR(a.date_last_modified,'YYYY-MM-DD HH24:MI:SS') AS date_last_modified,
       DECODE(c.name,
             'NZ - ANZ Current Account','DP_HDR',
             decode(a.transaction_line_id,0,'DP_HDR',decode(d.transaction_line_id,NULL,decode(i.item_id,NULL,'DP_LINE','DP_TAX'),'DP_LINE'))
       ) AS line_type
FROM transaction_lines a
  INNER JOIN transactions b ON (TRANSACTION_LINES.transaction_id = transactions.transaction_id)
  LEFT OUTER JOIN accounts c ON (TRANSACTION_LINES.account_id = accounts.account_id)
...
AND EXISTS (SELECT 1
              FROM transactions i
              WHERE i.transaction_id = a.transaction_id
              AND   i.transaction_type = 'Deposit'
	      AND  i.date_last_modified >= to_timestamp('%s','YYYY-MM-DD HH24:MI:SS'))
UNION ALL
SELECT b.tranid AS document_number,
       b.transaction_number AS transaction_number,
       ...
       TO_CHAR(B.entity_ID) AS customer_ID,
       TO_CHAR(a.price_type_ID) AS price_type_ID,
       TO_CHAR(NVL(B.custom_form_ID,118)) AS custom_form_ID,
       TO_CHAR(b.created_by_ID) AS created_by_ID,
       TO_CHAR(B.create_date,'YYYY-MM-DD HH24:MI:SS') AS create_date,
       TO_CHAR(a.date_last_modified,'YYYY-MM-DD HH24:MI:SS') AS date_last_modified,
       DECODE(c.name,
             'AP-General','BL_HDR',
             decode(a.transaction_line_id,0,'BL_HDR',decode(d.transaction_line_id,NULL,decode(i.item_id,NULL,'BL_LINE','BL_TAX'),'BL_LINE'))
       ) AS line_type
FROM transaction_lines a
  INNER JOIN transactions b ON (TRANSACTION_LINES.transaction_id = transactions.transaction_id)
  LEFT OUTER JOIN accounts c ON (TRANSACTION_LINES.account_id = accounts.account_id)
