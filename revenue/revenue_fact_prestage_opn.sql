...
,QUANTITY                   
 ...
 INNER JOIN DW.transaction_type n ON (NVL(A.ref_custom_form_id,-99) = n.transaction_type_id) 
 INNER JOIN DW.transaction_status o ON (NVL(A.STATUS,'NA_GDW') = o.status AND NVL(A.TRANSACTION_TYPE,'NA_GDW') = o.DOCUMENT_TYPE)
 INNER JOIN DW.transaction_type p ON (NVL(A.custom_form_id,-99) = p.transaction_type_id) 
 INNER JOIN DW.accounting_period q ON (NVL(A.accounting_period_id,-99) = q.accounting_period_id)   
...
,SYSDATE AS DW_CREATION_DATE
 from dw_prestage.revenue_fact a /* 18555 */
 LEFT OUTER JOIN DW.PAYMENT_TERMS b ON (NVL (A.PAYMENT_TERMS_ID,-99) = b.PAYMENT_TERMS_ID)
 LEFT OUTER JOIN DW.territories c ON (NVL (A.sales_rep_ID,-99) = c.territory_ID)
 LEFT OUTER JOIN DW.CURRENCIES d ON (NVL (A.CURRENCY_ID,-99) = d.CURRENCY_ID)
 LEFT OUTER JOIN DW.DWDATE e ON (NVL (TO_CHAR (A.tranDATE,'YYYYMMDD'),'0') = e.DATE_ID)
 LEFT OUTER JOIN DW.ACCOUNTS F ON (NVL (A.account_ID,-99) = f.account_ID)
 LEFT OUTER JOIN DW.ITEMS g ON (NVL (A.ITEM_ID,-99) = g.ITEM_ID)
 LEFT OUTER JOIN DW.TAX_ITEMS h ON (NVL (A.TAX_ITEM_ID,-99) = h.ITEM_ID)
 LEFT OUTER JOIN DW.SUBSIDIARIES j ON (NVL (A.SUBSIDIARY_ID,-99) = j.SUBSIDIARY_ID)  
 LEFT OUTER JOIN DW.LOCATIONS k ON (NVL (A.LOCATION_ID,-99) = k.LOCATION_ID)
 LEFT OUTER JOIN DW.CLASSES l ON (NVL(A.CLASS_ID,-99) = l.CLASS_ID) 
 LEFT OUTER JOIN DW.customers m ON (NVL(A.customer_ID,-99) = m.customer_ID) 
 LEFT OUTER JOIN DW.transaction_type n ON (NVL(A.ref_custom_form_id,-99) = n.transaction_type_id) 
 LEFT OUTER JOIN DW.transaction_status o ON (NVL(A.STATUS,'NA_GDW') = o.status AND NVL(A.TRANSACTION_TYPE,'NA_GDW') = o.DOCUMENT_TYPE)
 LEFT OUTER JOIN DW.transaction_type p ON (NVL(A.custom_form_id,-99) = p.transaction_type_id) 
 LEFT OUTER JOIN DW.accounting_period q ON (NVL(A.accounting_period_id,-99) = q.accounting_period_id)   
