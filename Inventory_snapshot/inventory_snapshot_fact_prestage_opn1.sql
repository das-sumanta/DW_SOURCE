...
SELECT SUBSIDIARY_ID  
      ,LOCATION_ID    
      ,ITEM_ID  
FROM (SELECT inventory_snapshot_ID
...
FROM dw_prestage.inventory_snapshot_fact_insert A
    ...
    INNER JOIN DW_REPORT.LOCATIONS C ON (NVL (A.LOCATION_ID,-99) = B.LOCATION_ID) 
...
FROM dw_prestage.inventory_snapshot_fact_insert A
    ...
    LEFT OUTER JOIN DW_REPORT.LOCATIONS C ON (A.LOCATION_ID = B.LOCATION_ID) 
...
WHERE dw.inventory_snapshot_fact.SUBSIDIARY_ID = dw_prestage.inventory_snapshot_fact_update.SUBSIDIARY_ID
and dw.inventory_snapshot_fact.LOCATION_ID = dw_prestage.inventory_snapshot_fact_update.LOCATION_ID
and dw.inventory_snapshot_fact.ITEM_ID = dw_prestage.inventory_snapshot_fact_update.ITEM_ID);
