...
SELECT A.DOCUMENT_NUMBER,
       ...
       C.TERRITORY_KEY,
       R.EMPLOYEE_KEY AS BOOK_FAIRS_CONSULTANT_KEY,
...
FROM dw_prestage.opportunity_fact_insert A
  ...
  INNER JOIN DW.DWDATE I ON (NVL(TO_CHAR (A.OPEN_DATE,'YYYYMMDD'),'19000101') = I.DATE_ID)
  INNER JOIN DW.DWDATE J ON (NVL(TO_CHAR (A.SHIP_DATE,'YYYYMMDD'),'19000101') = J.DATE_ID)
  INNER JOIN DW.DWDATE K ON (NVL(TO_CHAR (A.CLOSE_DATE,'YYYYMMDD'),'19000101') = K.DATE_ID)
  INNER JOIN DW.DWDATE L ON (NVL(TO_CHAR (A.RETURN_DATE,'YYYYMMDD'),'19000101') = L.DATE_ID)
...
SELECT A.RUNID,
       ...
       CASE
         WHEN (R.EMPLOYEE_KEY IS NULL AND A.BOOK_FAIRS_CONSULTANT_ID IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN (R.EMPLOYEE_KEY IS NULL AND A.BOOK_FAIRS_CONSULTANT_ID IS NULL) THEN ' NO DIM FROM SOURCE '
         ELSE 'OK'
       END,
       D.EMPLOYEE_KEY AS SALES_REP_KEY,
       CASE
       ...
       CASE
         ...
         WHEN (M.TRANSACTION_TYPE_KEY IS NULL AND A.CUSTOM_FORM_ID IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END,
       A.FORECAST_TYPE,
       G.CURRENCY_KEY,
       ...
       H.DATE_KEY AS TRANSACTION_DATE_KEY,
       CASE
         WHEN (H.DATE_KEY IS NULL AND A.TRANDATE IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN (H.DATE_KEY IS NULL AND A.TRANDATE IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END,
       I.DATE_KEY AS OPEN_DATE_KEY,
       CASE
         WHEN (I.DATE_KEY IS NULL AND A.OPEN_DATE IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN (I.DATE_KEY IS NULL AND A.OPEN_DATE IS NULL) THEN ' NO DIM FROM SOURCE '
         ELSE 'OK'
       END,
       J.DATE_KEY AS SHIP_DATE_KEY,
       CASE
         WHEN (J.DATE_KEY IS NULL AND A.SHIP_DATE IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN (J.DATE_KEY IS NULL AND A.SHIP_DATE IS NULL) THEN ' NO DIM FROM SOURCE '
         ELSE 'OK'
       END,
       K.DATE_KEY AS CLOSE_DATE_KEY,
       CASE
         WHEN (K.DATE_KEY IS NULL AND A.CLOSE_DATE IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN (K.DATE_KEY IS NULL AND A.CLOSE_DATE IS NULL) THEN ' NO DIM FROM SOURCE '
         ELSE 'OK'
       END,
       L.DATE_KEY AS RETURN_DATE_KEY,
       CASE
         WHEN (L.DATE_KEY IS NULL AND A.RETURN_DATE IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN (L.DATE_KEY IS NULL AND A.RETURN_DATE IS NULL) THEN ' NO DIM FROM SOURCE '
         ELSE 'OK'
       END,
       A.EXCHANGE_RATE,
       A.ROLL_SIZE,
       ...
       CASE
         ...
         WHEN (P.SUBSIDIARY_KEY IS NULL AND A.SUBSIDIARY_ID IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END,
       Q.CUSTOMER_KEY,
       CASE
         WHEN (Q.CUSTOMER_KEY IS NULL AND A.CUSTOMER_ID IS NOT NULL) THEN ' DIM LOOKUP FAILED '
         WHEN (Q.CUSTOMER_KEY IS NULL AND A.CUSTOMER_ID IS NULL) THEN ' NO MANDATORY DIM FROM SOURCE FAILED '
         ELSE 'OK'
       END,
       A.DATE_LAST_MODIFIED,
       A.SALES_REP_ID,         
...
FROM dw_prestage.opportunity_fact_insert A
  ...
  LEFT OUTER JOIN DW.DWDATE I ON (NVL(TO_CHAR (A.OPEN_DATE,'YYYYMMDD'),'19000101') = I.DATE_ID)
  LEFT OUTER JOIN DW.DWDATE J ON (NVL(TO_CHAR (A.SHIP_DATE,'YYYYMMDD'),'19000101') = J.DATE_ID)
  LEFT OUTER JOIN DW.DWDATE K ON (NVL(TO_CHAR (A.CLOSE_DATE,'YYYYMMDD'),'19000101') = K.DATE_ID)
  LEFT OUTER JOIN DW.DWDATE L ON (NVL(TO_CHAR (A.RETURN_DATE,'YYYYMMDD'),'19000101') = L.DATE_ID)
  LEFT OUTER JOIN DW_REPORT.TRANSACTION_TYPE M ON (A.CUSTOM_FORM_ID = M.TRANSACTION_TYPE_ID)
  LEFT OUTER JOIN DW_REPORT.LOCATIONS N ON (A.LOCATION_ID = N.LOCATION_ID)
  ...
  LEFT OUTER JOIN DW_REPORT.EMPLOYEES R ON (A.BOOK_FAIRS_CONSULTANT_ID = R.EMPLOYEE_ID)
  WHERE 
  ...
  Q.CUSTOMER_KEY IS NULL OR
  (R.EMPLOYEE_KEY IS NULL AND A.BOOK_FAIRS_CONSULTANT_ID IS NOT NULL ));
