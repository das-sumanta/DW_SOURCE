...
SELECT
 ...
 FROM DW.STANDING_ORDER_SCHEDULE_FACT_ERROR A
    ...
    INNER JOIN DW_REPORT.PRODUCT_CATALOGUE E ON (NVL (A.PRODUCT_CATALOGUE_ID,-99) = E.PRODUCT_CATALOGUE_ID)
...
UPDATE DW_PRESTAGE.STANDING_ORDER_SCHEDULE_FACT_ERROR
 SET RECORD_STATUS = 'INSERT'
 WHERE NOT EXISTS
 (SELECT 1 FROM DW_REPORT.STANDING_ORDER_SCHEDULE_FACT B
 WHERE DW_PRESTAGE.STANDING_ORDER_SCHEDULE_FACT_ERROR.STANDING_ORDER_SCHEDULE_ID = B.STANDING_ORDER_SCHEDULE_ID
 AND DW_PRESTAGE.STANDING_ORDER_SCHEDULE_FACT_ERROR.SUBSIDIARY_KEY = B.SUBSIDIARY_KEY );
...
AND   EXISTS (SELECT 1 FROM dw_prestage.STANDING_ORDER_SCHEDULE_FACT_ERROR
  ...
  AND dw.STANDING_ORDER_SCHEDULE_fact.subsidiary_key = dw_prestage.STANDING_ORDER_SCHEDULE_FACT_ERROR.subsidiary_key
...
where exists ( select 1 from dw_prestage.STANDING_ORDER_SCHEDULE_fact_error b
  ...
  AND dw.STANDING_ORDER_SCHEDULE_fact_error.STANDING_ORDER_SCHEDULE_id = b.STANDING_ORDER_SCHEDULE_id);
